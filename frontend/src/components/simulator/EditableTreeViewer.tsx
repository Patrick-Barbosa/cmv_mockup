...
export function EditableTreeViewer({
  ...
  const [composicaoCost, setComposicaoCost] = useState<number>(0)
  ...
  const recalcCost = async (list: Componente[]) => {
    setCalculating(true)
    try {
      const mapToPayload = (compList: Componente[]): any[] =>
        compList
          ...
          .map((c) => ({
            ...
            tipo: c.tipo,
            sub_componentes: c.subComponentes && c.subComponentes.length > 0
              ? mapToPayload(c.subComponentes)
              : undefined,
          }))
      const payload = mapToPayload(list)
      if (payload.length === 0) {
        setComposicaoCost(0)
        return
      }
      const result = await simulatorApi.calculateCost({ componentes: payload })
      setComposicaoCost(result.total_cost)
    } catch {
      setComposicaoCost(0)
    } finally {
